...
class Program
{
    static void Main(string[] args) {
        ...
        string inputString = lines[0].Trim().Replace("k", "");// Зчитуємо текстовий рядок і видаляємо всі "k"
        Console.WriteLine("Введений рядок з файлу (без букви 'k'): " + inputString);

        int desiredLength;                                    // Зчитуємо довжину другого рядка
        if (!int.TryParse(lines[1].Trim(), out desiredLength) || desiredLength < inputString.Length) {
        ...
        int spacesBefore = (desiredLength - inputString.Length) / 2;                // Обчислюємо позицію для вставлення
        char[] resultArray = new string('0', desiredLength).ToCharArray();          // Створюємо рядок з нулями

        for (int i = 0; i < inputString.Length; i++) {                              // Вставляємо текст першого рядка в центр другого, рахуючи з кінця
